...
/// Set task priority to "later" (local-first with optional sync).
pub async fn run(config: &Config, task_id: &str, no_sync: bool) -> Result<()> {
    ...
    // Set priority to "later"
    task.priority = "later".to_string();
    ...
    println!("{}", "✓ Task priority updated locally!".green().bold());
