...
pub async fn run(
            ...
            for label in &labels {
                if !project_labels.contains(label) {
                    let project_name = ctx
                        ...
                        .map(|path| path.join("/"))
                        .unwrap_or_else(|_| task.project_id.clone());
        ...
        && task.project_id != *new_project
    {
        ...
        let missing = crate::labels::find_missing_labels(&task.labels, new_project, &ctx.cache)?;
        if !missing.is_empty() {
            ...
            if !labels_to_create.is_empty() {
                if !no_sync {
                        ...
                        .await
                    {
                        ...
                        Err(_) => {
                            let mut all_labels = ctx.cache.get_effective_labels(new_project)?;
